#!/usr/bin/env bun
/**
 * Plan, implement, and update Teamwork task workflow
 *
 ...
 * 3. /update_teamwork_task - Update Teamwork task with result
 *
 * Usage:
 *   bun run adws/adw_plan_implement_update_teamwork_task.ts --adw-id abc123 --worktree-name app --task "Build todo app" --task-id 123 --prototype vite_vue
 */
...
if (!adwId || !worktreeName || !task || !taskId) {
  ...
  console.error("Usage: bun run adw_plan_implement_update_teamwork_task.ts --adw-id <id> --worktree-name <name> --task <desc> --task-id <id> [--prototype <type>]");
  process.exit(1);
}
...
console.log("\n╔═══════════════════════════════════════════════════════════╗");
console.log("║      Teamwork Plan-Implement-Update Workflow             ║");
console.log("╚═══════════════════════════════════════════════════════════╝\n");
...
// =============================================================================
// Phase 3: Update Teamwork Task
// =============================================================================
console.log("\n─────────────────────────────────────────────────────────────");
console.log("Phase 3: Update Teamwork Task (/update_teamwork_task)");
console.log("─────────────────────────────────────────────────────────────\n");
...
await Bun.write(
  workflowSummaryPath,
  JSON.stringify({
    workflow: "plan_implement_update_teamwork_task",
    adw_id: adwId,
    worktree_name: sanitizedWorktree,
    task,
    task_id: taskId,
