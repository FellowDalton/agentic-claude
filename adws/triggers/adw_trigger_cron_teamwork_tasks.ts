 ...
 *
 * Usage:
 *   bun run adws/triggers/adw_trigger_cron_teamwork_tasks.ts [--once] [--dry-run] [--interval 15] [--max-tasks 3]
 */
...
// Get Teamwork project ID from environment
const projectId = process.env["TEAMWORK_PROJECT_ID"];
if (!projectId) {
  logger.error("Error: TEAMWORK_PROJECT_ID environment variable not set");
  process.exit(1);
}

logger.info("Teamwork Task Monitor starting...");
...
/**
 * Fetch eligible tasks from Teamwork
 */
async function getEligibleTasks(): Promise<TeamworkTask[]> {
  logger.info("Fetching tasks from Teamwork...");

  const request: AgentTemplateRequest = {
    agent_name: "teamwork-task-fetcher",
    slash_command: "/get_teamwork_tasks",
    args: [projectId, JSON.stringify(["New", "To Do", "Review"]), "10"],
    adw_id: makeAdwId(),
    model: "sonnet",
  ...
  try {
    ...
    for (const taskDict of taskData) {
      try {
        ...
        if (TeamworkTaskHelpers.isEligibleForProcessing(teamworkTask)) {
...
async function updateTaskStatus(
  ...
  const request: AgentTemplateRequest = {
    ...
    args: [taskId, status, updateContent],
    adw_id: makeAdwId(),
    model: "sonnet",
    working_dir: process.cwd(),
  };
  ...
  try {
    ...
    if (response.success) {
      logger.success(`Successfully updated task ${taskId} to ${status}`);
      return true;
    } else {
 ...
 */
function generateWorktreeName(task: TeamworkTask): string {
 ...
 */
function determineWorkflow(task: TeamworkTask): "build" | "plan-implement" {
 ...
 */
async function delegateTask(task: TeamworkTask): Promise<void> {
  // Generate ADW ID
  ...
  const model = TeamworkTaskHelpers.getPreferredModel(task);

  // Get worktree name
  const worktreeName = generateWorktreeName(task);

  // Get task prompt
  const taskPrompt = TeamworkTaskHelpers.getTaskPromptForAgent(task);
  ...
  const workflowScript =
    workflow === "build"
      ? "adw_build_update_teamwork_task.ts"
      : "adw_plan_implement_update_teamwork_task.ts";
  ...
  logger.success(`Task ${task.task_id} delegated successfully (PID: ${proc.pid})`);
}
