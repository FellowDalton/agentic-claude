...
description: Complete storyblok-integrations workflow - plan, build, and self-improve
argument-hint: [request] [human_in_the_loop (bool)]
---

# Purpose

Orchestrates complete storyblok-integrations implementation cycle: plan -> build -> self-improve.
...
Task(
  ...
  prompt: "Run SlashCommand('/skills:storyblok-integrations:plan [USER_PROMPT]'). Return the path to the generated plan file."
)
```
...
Task(
  subagent_type: "general-purpose",
  prompt: "Run SlashCommand('/skills:storyblok-integrations:self-improve true'). Return the self-improvement report."
)
```
...
### Final Status
storyblok-integrations implementation workflow complete
