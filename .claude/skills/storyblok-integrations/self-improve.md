...
description: Self-improve storyblok-integrations expertise by validating against codebase
argument-hint: [check_git_diff (bool)] [focus_area (optional)]
---

# Purpose

Maintain storyblok-integrations expertise accuracy by comparing against actual implementation. Learnings are always written to the JSONL journal — never directly to the expertise YAML. An orchestrator consolidates the journal into expertise.yaml separately.
...
JOURNAL_FILE: .claude/skills/storyblok-integrations/expertise.learnings.jsonl

## Instructions

- This is a self-improvement workflow to keep storyblok-integrations expertise synchronized with the actual codebase
- Think of the expertise file as your **mental model** and memory reference for all storyblok-integrations-related functionality
...
```json
{"ts":"ISO-8601-timestamp","expert":"storyblok-integrations","track":"<track-id>","story":"<story-slug>","type":"update|add|remove","section":"<yaml-section-name>","description":"<what changed and why>","content":"<the actual YAML snippet or value to add/change/remove>"}
```

- `ts`: current UTC timestamp in ISO-8601 format
- `expert`: the expert name (`storyblok-integrations`)
