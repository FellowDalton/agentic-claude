...
description: Complete storyblok-admin workflow - plan, build, and self-improve
argument-hint: [request] [human_in_the_loop (bool)]
---

# Purpose

Orchestrates complete storyblok-admin implementation cycle: plan -> build -> self-improve.
...
Task(
  ...
  prompt: "Run SlashCommand('/skills:storyblok-admin:plan [USER_PROMPT]'). Return the path to the generated plan file."
)
```
...
Task(
  subagent_type: "general-purpose",
  prompt: "Run SlashCommand('/skills:storyblok-admin:self-improve true'). Return the self-improvement report."
)
```
...
### Final Status
storyblok-admin implementation workflow complete
