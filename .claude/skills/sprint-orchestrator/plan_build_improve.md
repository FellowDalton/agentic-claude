...
description: Complete sprint-orchestrator workflow - plan, build, and self-improve
argument-hint: [request] [human_in_the_loop (bool)]
---

# Purpose

Orchestrates complete sprint-orchestrator implementation cycle: plan -> build -> self-improve.
...
Task(
  ...
  prompt: "Run SlashCommand('/skills:sprint-orchestrator:plan [USER_PROMPT]'). Return the path to the generated plan file."
)
```
...
Task(
  subagent_type: "general-purpose",
  prompt: "Run SlashCommand('/skills:sprint-orchestrator:self-improve true'). Return the self-improvement report."
)
```
...
### Final Status
sprint-orchestrator implementation workflow complete
