...
description: Self-improve frontend-tester expertise by validating against codebase
argument-hint: [check_git_diff (bool)] [focus_area (optional)]
---

# Purpose

Maintain frontend-tester expertise accuracy by comparing against actual implementation. Learnings are always written to the JSONL journal — never directly to the expertise YAML. An orchestrator consolidates the journal into expertise.yaml separately.

## Variables

CHECK_GIT_DIFF: $1 default to false if not specified
FOCUS_AREA: $2 default to empty string
EXPERTISE_FILE: .claude/skills/frontend-tester/expertise.yaml
JOURNAL_FILE: .claude/skills/frontend-tester/expertise.learnings.jsonl

## Instructions

- This is a self-improvement workflow to keep frontend-tester expertise synchronized with the actual codebase
- Think of the expertise file as your **mental model** and memory reference for all frontend-tester-related functionality
...
```json
{"ts":"ISO-8601-timestamp","expert":"frontend-tester","track":"<track-id>","story":"<story-slug>","type":"update|add|remove","section":"<yaml-section-name>","description":"<what changed and why>","content":"<the actual YAML snippet or value to add/change/remove>"}
```

- `ts`: current UTC timestamp in ISO-8601 format
- `expert`: the expert name (`frontend-tester`)
