...
description: Complete sprint-reporter workflow - plan, build, and self-improve
argument-hint: [request] [human_in_the_loop (bool)]
---

# Purpose

Orchestrates complete sprint-reporter implementation cycle: plan -> build -> self-improve.
...
Task(
  ...
  prompt: "Run SlashCommand('/skills:sprint-reporter:plan [USER_PROMPT]'). Return the path to the generated plan file."
)
```
...
Task(
  subagent_type: "general-purpose",
  prompt: "Run SlashCommand('/skills:sprint-reporter:self-improve true'). Return the self-improvement report."
)
```
...
### Final Status
sprint-reporter implementation workflow complete
