...
description: Complete ibexa-format workflow - plan, build, and self-improve
argument-hint: [request] [human_in_the_loop (bool)]
---

# Purpose

Orchestrates complete ibexa-format implementation cycle: plan -> build -> self-improve.
...
Task(
  ...
  prompt: "Run SlashCommand('/skills:ibexa-format:plan [USER_PROMPT]'). Return the path to the generated plan file."
)
```
...
Task(
  subagent_type: "general-purpose",
  prompt: "Run SlashCommand('/skills:ibexa-format:self-improve true'). Return the self-improvement report."
)
```
...
### Final Status
ibexa-format implementation workflow complete
