...
2. **Detection**: Identifies tasks with status "New" or "Review" + `execute` trigger
3. **Claiming**: Updates status to "In progress" with ADW ID to prevent duplicate work
4. **Routing**: Routes to workflow based on `prototype:type` tags or `workflow:plan` tags
...
execute_template(AgentTemplateRequest(
    ...
    args=[task_id, "Done", json.dumps({"commit": commit_hash})],
    adw_id=adw_id
))
```
...
4. Test with a Teamwork task using native tag `prototype:[prototype]`
...
```bash
# 1. Find the ADW ID from Teamwork task (in "In progress" status details)
adw_id="abc12345"
...
1. Check status is "New" or "Review"
